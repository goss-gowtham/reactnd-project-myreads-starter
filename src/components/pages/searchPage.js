...
class searchPage extends React.Component {
  ...
  updateBook = (book,shelf) => {
    ...
    .then(resp => {
      book.shelf = shelf;
      this.setState(state => ({
        books: state.books.filter(b => b.id !== book.id).concat([book]) /*Concatenating to the API with the respective book by it's id*/
      }))
    });
  }
  updateQuery = (query) => {
  ...
  submitSearch() {
    ...
    BooksAPI.search(this.state.query.trim()).then(resp => {
      if(resp.error) {
        ...
        resp.forEach(
          b => {
            let find = this.state.books.filter(boo => boo.id === b.id);   /*Check book is same as the response book*/
